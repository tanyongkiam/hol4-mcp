...
Unqualified `§` names are sections of [[feedback_hol4_mcp_proving]]. H6 injects three of these rows at the moment of failure, so those three reach you without being remembered — the rest do not, which is why the index above is a rule and not a convenience.
...
- **Gate 1 — Dev-scaffolding `suspend`/`Resume`/`Finalise` is ALWAYS BAD JUNK; inline it back.** The default finished form of ANY theorem is ONE `Proof … QED` with **no `suspend`/`Resume`/`Finalise`**. Any you created to navigate or develop MUST be gone from the committed form.
  - **Survives in EXACTLY TWO cases, nothing else**: **(a)** a genuine multi-case **induction** (`recInduct`/`Induct`) — or a similarly large multi-arm split — kept as one `Resume`-per-case (the flat-ladder TOC); **(b)** explicit user approval (mandatory for any level-2 sub-suspend — ≤2-deep, never deeper).
...
- **End-of-theorem — confirmation only**, after every chunk was already stepped (you should know it's OK). Either form of step 1 of the End-of-proof verification ladder below counts; `hol_check_proof` = `Status: OK` is the usual one, and the `hol_state_at`-past-`QED` form is the fallback when a legitimately slow proof times out. If neither can confirm — PROMPT THE USER; do NOT substitute `holmake` (that's the file gate, not a per-theorem gate).
...
- **A navigation/check TIMEOUT is YOUR looping tactic, not a slow prefix** — diagnose per the bullet above (inside a chain → sub-suspend; cheat-the-frontier only on a flat body), don't widen `timeout=`. [[feedback_replay_discipline]] §TIMEOUT.
...
- `TRY` / `ORELSE` / `FIRST` (hide failure — `FIRST` is `ORELSE` over a list): existing uses → restructure to `>~ [pat] >- suspend "X"` dispatch before modifying any body. Tempted to add a fresh one? ask "what shape is THIS goal, what closes it?" — almost always a `>~ [pat]` selector or a `Cases_on`.
- `>|` (`THENL`) (position-keyed brittleness): split per-subgoal via suspend/Resume or per-case `>~`/`>-`, not by goal-stack index.

H1 blocks all four at edit time in a `*Script.sml`.
