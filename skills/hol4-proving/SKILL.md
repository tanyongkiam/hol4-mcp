...
⛔ **A `git commit` recording proof code is also an enforced audit trigger.** H27 judges newly introduced Gates 1/2/3/5 and composition findings, not inherited unchanged debt. An exception requires explicit user approval of the displayed content-scoped review and finding class; style exceptions cannot admit proofs, and audit approval grants no Git authority. Review approval expires; see `hooks/README.md` in the hol4-mcp repo for scope/lifetime. Gate 6 stays yours to judge. A mechanical style finding is a review prompt, not proof that the theorem is defective.
...
- ⛔ **Repeated long TARGET replay needs a smaller replay unit.** For `SLOW NAVIGATION #n`, sub-suspend the failing arm before another expensive probe; structural induction dispatchers follow Gate 1. For **slow prefix/setup**, first inspect the active phase and checkpoint reuse: rebuilding already-built ancestors or splitting the target cannot fix current-file translation cost. Elapsed time alone does not justify rewriting passing proof arms. Phase/recovery details: [[feedback_replay_discipline]] §TIMEOUT; conversion craft: [[feedback_suspend_resume]].
