...
## TIMEOUT — diagnose the measured phase
Read `prefix=Ps` versus `target=Ts` and the active item before changing the proof or budget. `hol_sessions` exposes the current dependency, preceding theorem or top-level SML/translation span and elapsed command time without replay. Missing/stale dependencies need a build; already-built heavy dependencies may need a preloaded heap. Current-file translation needs prefix/checkpoint diagnosis, not another ancestor build or target-proof restructuring. A justified cold prefix can use a larger navigation budget; ordinary warm navigation should reuse its context.
...
- **For target replay, diagnose before widening the timeout**: if the failure is inside a `>-`/`THEN1`/`by (...)` chain, sub-suspend that arm; otherwise read the flat frontier before the failing tactic. Check looping rewrites and prover blowups before treating it as a legitimate expensive proof.
