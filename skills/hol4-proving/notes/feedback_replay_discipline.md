...
## ⛔ NEVER `hol_restart` without explicit user permission — ask first
`hol_state_at` auto-detects file edits and re-initialises its own replay — a restart after editing is never needed and the MCP server instructions say so explicitly. Do NOT restart pre-emptively. If a file-replay break looks nonsensical, READ it and reason about it; don't reach for restart. If you genuinely think a restart is warranted, ASK the user for permission and restart only if they allow it.
