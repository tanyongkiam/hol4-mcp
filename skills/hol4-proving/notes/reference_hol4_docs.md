...
| HOL4 technique | `~/hol4-mcp/skills/hol4-proving/notes/*.md` (this dir) | the detailed HOW the rules point to (tactic gotchas, dispatch/inline craft, replay discipline, plan pointers, unprovable-vs-unfound). One source per topic | on-demand (skill "Required reads") |
...
The whole HOL4 corpus lives in the `hol4-mcp` repo under `skills/hol4-proving/` — deliberately NOT under that repo's `.claude/`, so it is a proof-work skill available everywhere (via the `~/.claude/skills/hol4-proving` symlink) and not a project skill that auto-loads when developing the MCP server itself. HOL4 technique notes are no longer global memory: they are `notes/*.md` here, reached through the skill's "Required reads".
...
- tactic gotchas / prover-gen names / gvarify / irule traps → `feedback_hol4_mcp_proving`
- banned tactics, audit gates → skill (H1/H17 enforce)
- running-server install / `localfixes` branch / pytest caveat → `reference_hol4_mcp`
...
5. **Verify mechanically**: stale phrase gone; `[[links]]` resolve; no topic with two canonical homes; re-read changed skill/feedback sections end-to-end. Hooks: `python3 -c "import ast; ast.parse(open(f).read())"`.
